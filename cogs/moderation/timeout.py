...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**<:RomelUser:1294654665895579721> Target User:** [{self.user}](https://discord.com/users/{self.user.id})\n<:RomelMention:1294654604998475856> **User Mention:** {self.user.mention}\n**<:RomelDM:1295595078122999915> DM Sent:** {dm_status}\n**<:RomelReason:1295595129809141812> Reason:** {reason}", color=0x000000)
        ...
        embed.add_field(name="<:RomelMod:1295601558985379852> Moderator:", value=interaction.user.mention, inline=False)
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        try:
            await user.send(f"<a:Romel_WarnFlash:1272569018183843874> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:RomelUser:1294654665895579721> Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                          f"<:RomelMention:1294654604998475856> **User Mention:** {user.mention}\n"
                                          f"<:RomelDM:1295595078122999915> **DM Sent:** {dm_status}\n"
                                          f"**<:RomelReason:1295595129809141812> Reason:** {reason or 'None'}\n"
                                          f"**<:RomelTime:1294654567539277824> Duration:** {duration_text}",
                              color=self.color)
        embed.set_author(name=f"Successfully Muted {user.name}", icon_url=self.get_user_avatar(user))
        embed.add_field(name="<:RomelMod:1295601558985379852> Moderator:", value=ctx.author.mention, inline=False)
        embed.set_footer(text=f"Requested by {ctx.author}", icon_url=self.get_user_avatar(ctx.author))
        embed.timestamp = discord.utils.utcnow()
