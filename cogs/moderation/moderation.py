...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
    ...
    deleted = len(deleted)
    messages = [f'<:3365:1373021326691930202> | {deleted} message{" was" if deleted == 1 else "s were"} removed.']
    if deleted:
        messages.append("")
    ...
    if len(to_send) > 2000:
        await ctx.send(f"<:3365:1373021326691930202> | Successfully removed {deleted} messages.", delete_after=7)
    else:
...
class Moderation(commands.Cog):
                  ...
                  if interaction.guild.me.guild_permissions.manage_roles:
                    ...
                    description=f"Unlocking all channels in {ctx.guild.name} .")
                      ...
                      await interaction.channel.send(
                              content=f"<:3365:1373021326691930202> | Successfully Unlocked {a} Channels")   
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                           "<:7942redmodshield:1373016146218848427> | It seems I'm missing the necessary permissions. Please grant me the `manage roles` permissions and try again.",
                              embed=None,
                                  view=None)
              else:
                    ...
                    description=f"Locking all channels in {ctx.guild.name}...")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:3365:1373021326691930202> | Successfully locked {a} Channels")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                    ...
                    description=f"Hiding all channels in {ctx.guild.name} ...")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:3365:1373021326691930202> | Successfully Hidden {a} Channel(s) .")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                    ...
                    description=f"Unhiding all channels in {ctx.guild.name} .")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:3365:1373021326691930202> | Successfully Unhidden {a} Channel(s) .")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
  ...
  async def _prefix(self, ctx: commands.Context, prefix: str):
      if not prefix:
          await ctx.reply(embed=discord.Embed(title="<:xo_cross:1373021326691930202> Error",
      ...
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          ...
          embed1=discord.Embed(title="<:3365:1373021326691930202> Success",
        ...
        if len(found_emojis) != 0:
            ...
            for emoji_id in found_emojis:
                try:
                    ...
                    continue  
            await init_message.delete()
            return await ctx.reply(f"<:3365:1373021326691930202> | Successfully deleted {delete_count}/{len(found_emojis)} emoji(s).")
  ...
  async def roleicon(self, ctx: commands.Context, role: discord.Role, *, icon: Union[discord.Emoji, discord.PartialEmoji, str] = None):
    ...
    if isinstance(icon, discord.Emoji) or isinstance(icon, discord.PartialEmoji):
        ...
        try:
            ...
            await role.edit(display_icon=image_data)
            success_embed = discord.Embed(
                description=f"<:3365:1373021326691930202> | Successfully changed the icon for {role.mention}.",
                color=self.color
            )
            return await ctx.reply(embed=success_embed, mention_author=False)
        except Exception as e:
      ...
      if interaction.user == ctx.author:
        if interaction.guild.me.guild_permissions.ban_members:
          ...
          async for idk in interaction.guild.bans(limit=None):
            ...
            a += 1
          await interaction.channel.send(
            content=f"<:3365:1373021326691930202> Successfully Unbanned {a} Members")
        else:
          await interaction.response.edit_message(
            content=
