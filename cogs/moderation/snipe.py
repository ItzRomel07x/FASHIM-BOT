...
class SnipeView(discord.ui.View):
    ...
    async def send_snipe_embed(self, interaction: discord.Interaction):
        ...
        embed.description = (
            f"<:RomelUser:1294654665895579721> **Author:** **[{display_name}](https://discord.com/users/{uid})**\n"
            f"<:RomelID:1294654633821863967> **Author ID:** `{snipe['author_id']}`\n"
            f"<:RomelMention:1294654604998475856> **Author Mention:** <@{snipe['author_id']}>\n"
            f"<:RomelTime:1294654567539277824> **Deleted:** <t:{snipe['deleted_at']}:R>\n"
        )

        if snipe['content']:
            embed.add_field(name="<:Romel_bin:1254336650075701308> **Content:**", value=snipe['content'])
        if snipe['attachments']:
            attachment_links = "\n".join([f"[{attachment['name']}]({attachment['url']})" for attachment in snipe['attachments']])
            embed.add_field(name="<:RomelMedia:1294574309565534229> **Attachments:**", value=attachment_links)
    ...
    @discord.ui.button(emoji="<:Romel_first:1254347208254558279>", style=discord.ButtonStyle.secondary, custom_id="first")
    async def first_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.index = 0
        self.update_buttons()
        await self.send_snipe_embed(interaction)

    @discord.ui.button(emoji="<:Romel_backward:1254314965351923795>", style=discord.ButtonStyle.secondary, custom_id="previous")
    ...
    @discord.ui.button(emoji="<:Romel_bin:1254336650075701308>", style=discord.ButtonStyle.danger, custom_id="delete")
    async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.message.delete()

    @discord.ui.button(emoji="<:Romel_forward:1254314940668317717>", style=discord.ButtonStyle.secondary, custom_id="next")
    ...
    @discord.ui.button(emoji="<:Romel_last:1254347295122526208>", style=discord.ButtonStyle.secondary, custom_id="last")
...
class Snipe(commands.Cog):
    ...
    async def snipe(self, ctx):
        ...
        embed.description = (
            f"<:RomelUser:1294654665895579721> **Author:** **[{display_name}](https://discord.com/users/{uid})**\n"
            f"<:RomelID:1294654633821863967> **Author ID:** `{first_snipe['author_id']}`\n"
            f"<:RomelMention:1294654604998475856> **Author Mention:** <@{first_snipe['author_id']}>\n"
            f"<:RomelTime:1294654567539277824> **Deleted:** <t:{first_snipe['deleted_at']}:R>\n"
        )

        if first_snipe['content']:
            embed.add_field(name="<:Romel_bin:1254336650075701308> **Content:**", value=first_snipe['content'])
