...
class NotifCommands(commands.Cog):
    ...
    async def twitch(self, ctx, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:3365:1373021326691930202> Success", description=f"Twitch notifications set for {role.mention} in {channel.mention}.", color=0x000000))
    ...
    async def youtube(self, ctx, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:3365:1373021326691930202> Success", description=f"YouTube notifications set for {role.mention} in {channel.mention}.", color=0x000000))
    ...
    async def reset(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.commit()
            await ctx.send(embed=discord.Embed(title="<:3365:1373021326691930202> Success", description="Twitch and YouTube notifications have been reset.", color=0x00FF00))
