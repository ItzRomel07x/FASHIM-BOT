...
from core import Cog, Romel, Context
...
class MusicControlView(View):
    ...
    @discord.ui.button(emoji="<:Play:1375130631926841354>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:Previous:1375131121888657558>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:Pause:1375131545882591324>", style=discord.ButtonStyle.success)
    async def pause_button(self, interaction: discord.Interaction, button: Button):
        if self.player.paused:
            await self.player.pause(False)

            await self.player.channel.edit(status=f"<:RightArrow:1374976222672715857> Playing: {self.player.current.title}")
            button.emoji = "<:Pause:1375131545882591324>" 
            await interaction.response.edit_message(view=self)

        elif self.player.playing:
            await self.player.pause(True)
            await self.player.channel.edit(status=f"<:RightArrow:1374976222672715857> Paused: {self.player.current.title}")
            button.emoji = "<:Play:1375130631926841354>"
            await interaction.response.edit_message(view=self)


    @discord.ui.button(emoji="<:Skip:1375132387901444207>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:Loop:1375132810624503808>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:Shuffle:1375133147980628092>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:Pause:1375131545882591324>", style=discord.ButtonStyle.secondary)
...
class Music(commands.Cog):
    def __init__(self, client: Romel):
        self.client = client
        ...
        if len(guild.voice_channels[0].members) == 1:
            ...
            if player:
                ...
                try:
                    ...
                    ended.set_footer(text="Thanks for choosing ELITE!")
                    support = Button(label='Support',
                                 style=discord.ButtonStyle.link,
                        url=f'https://discord.gg/7C5hMj2Uah')
                    vote = Button(label='Youtube',
                                 style=discord.ButtonStyle.link,
                        url=f'https://www.youtube.com/@elite_x_999')
                    view = View()
    ...
    async def play_source(self, ctx, query):
        if not ctx.author.voice:
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> You need to be in a voice channel to use this command.", color=0x000000))
            return
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(embed=discord.Embed(description=f"<:RightArrow:1374976222672715857> Added playlist [{tracks.name}](https://discord.gg/7C5hMj2Uah) with **{len(tracks.tracks)} songs** to the queue.", color=0x000000))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(embed=discord.Embed(description=f"<:RightArrow:1374976222672715857> Added [{track.title}](https://discord.gg/7C5hMj2Uah) to the queue.", color=0x000000))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(embed=discord.Embed(description=f"<:RightArrow:1374976222672715857> Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://discord.gg/7C5hMj2Uah)** to the queue.", color=0x000000))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"<:RightArrow:1374976222672715857> Added all tracks from album **[{album_info['name']}](https://discord.gg/7C5hMj2Uah)** to the queue.", color=0x000000))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def autoplay(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> No song is currently playing.", color=0x000000))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> You need to be in the same voice channel as me to use this command.", color=0xFF0000))
            return
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:Tick:1374999784242544660>  Autoplay {'enabled' if vc.autoplay == wavelink.AutoPlayMode.enabled else 'disabled'} by {ctx.author.mention}.", color=0x000000))
    ...
    async def loop(self, ctx: commands.Context):
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:Tick:1374999784242544660>  Loop {'enabled' if vc.queue.mode == wavelink.QueueMode.loop else 'disabled'} by {ctx.author.mention}.", color=0x000000))
        else:
    ...
    async def pause(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.paused:
            await vc.pause(True)
            await vc.channel.edit(status=f"<:Play:1375130631926841354> Paused: {vc.current.title}")
            await ctx.send(embed=discord.Embed(description=f"Paused by {ctx.author.mention}.", color=0x000000))
        else:
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> Nothing is playing or already paused.", color=0xFF0000))
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if vc and vc.paused:
            ...
            await vc.channel.edit(status=f"<:Play:1375130631926841354> Playing: {vc.current.title}")
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.queue.is_empty:
            ...
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> No song is playing or in the queue to skip.", color=0xFF0000))
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if not vc:
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> I'm not connected to a voice channel.", color=0xFF0000))
            return
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> Volume must be between 1 and 150.", color=0xFF0000))
        else:
    ...
    async def queue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> The queue is currently empty.", color=0x000000))
            return
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> No Queue to clear.", color=0xFF0000))
            return
    ...
    async def replay(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<a:Warn:1374998455520460932> I'm not connected to any voice channel.", color=0xFF0000))
            return
    ...
    async def on_wavelink_track_start(self, payload: wavelink.TrackStartEventPayload):
        ...
        if voice_channel:
            await voice_channel.edit(status=f"<:Play:1375130631926841354> Playing: {track.title}")  # type: ignore
...
@Author: Sonu Jana
    + Discord: me.sonu
    + Community: https://discord.gg/7C5hMj2Uah (Romel Development)
