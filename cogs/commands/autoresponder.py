    ...
    async def _create(self, ctx, name, *, message):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM autoresponses WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                ...
                if count >= 20:
                    return await ctx.reply(embed=discord.Embed(title="<:xo_cross:1373021326691930202> Error!",
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:3365:1373021326691930202> Success",
    ...
    async def _config(self, ctx):
        ...
        if not autoresponses:
            return await ctx.reply(embed=discord.Embed(
                description=f"<:7942redmodshield:1373016146218848427> | There are no autoresponders in {ctx.guild.name}",
                color=0x000000
            ))
