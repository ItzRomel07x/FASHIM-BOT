...
class Automod(commands.Cog):
    ...
    async def enable(self, ctx):
        ...
        if ctx.author != ctx.guild.owner and ctx.author.top_role.position < ctx.guild.me.top_role.position:
            embed = discord.Embed(title="<:xo_cross:1373021326691930202> Access Denied", description="Your top role must be at the **same** position or **higher** than my top role.", color=0x000000)
        ...
        if await self.is_automod_enabled(guild_id):
            embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"**<:xo_cross:1373021326691930202> Your Server already has Automoderation Enabled.**\n\nCurrent Status: <:3365:1373021326691930202> Enabled\nTo Disable use `{ctx.prefix}automod disable`", color=0x000000)
        ...
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        embed.description = "\n".join([f"<:xo_cross:1373021326691930202><:3365:1373021326691930202> : {event}" for event in events])
    ...
    async def enable_automod(self, ctx, guild_id, selected_events, interaction):
        ...
        embed.description = "\n".join([f"<:xo_cross:1373021326691930202><:3365:1373021326691930202> : {event}" for event in selected_events] +
                                       [f"<:xo_cross:1373021326691930202><:3365:1373021326691930202> : {event}" for event in ["Anti spam", "Anti caps", "Anti link", "Anti invites", "Anti mass mention", "Anti emoji spam", "Anti NSFW link"] if event not in selected_events])
        ...
        async def enable_logging_callback(interaction):
            ...
            try:
                for channel in interaction.guild.channels:
                    if channel.name == "Romel-automod":
                        await interaction.response.send_message(f"A logging channel with the name \"Romel-automod\" already exists.", ephemeral=True)
                        return
                log_channel = await interaction.guild.create_text_channel("Romel-automod", overwrites=overwrites)
    ...
    async def ignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            if await cursor.fetchone() is not None:
                embed = discord.Embed(title="__Channel Already Whitelisted!__", description=f"<:xo_cross:1373021326691930202> The channel {channel.mention} is already in the ignore list.\n\n➜ Use **{ctx.prefix}automod unignore channel {channel.mention}** to remove it.", color=0x000000)
            ...
            success = discord.Embed(title="<:3365:1373021326691930202> Channel Whitelisted", description=f"The channel {channel.mention} has been added to the ignore list \n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x000000)
    ...
    async def ignore_role(self, ctx, role: discord.Role):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            if await cursor.fetchone() is not None:
                embed = discord.Embed(title="__Role Already Whitelisted!__", description=f"<:xo_cross:1373021326691930202> The role {role.mention} is already in the ignore list.\n\n➜ Use **{ctx.prefix}automod unignore role {role.mention}** to remove it.", color=0x000000)
                embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                await ctx.send(embed=embed)
                return
            ...
            success = discord.Embed(title="<:3365:1373021326691930202> Role Whitelisted", description=f"The role {role.mention} has been added to the ignore list \n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x000000)
            success.set_thumbnail(url=self.bot.user.avatar.url)
            success.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
    ...
    async def ignore_reset(self, ctx):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            await db.commit()
        embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"** <:3365:1373021326691930202> | All ignored channels and roles have been reset!**\n\nTo view current Automod settings use `{ctx.prefix}automod config`", color=0x000000)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
               icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
    ...
    async def unignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="<:3365:1373021326691930202> Success", description=f"{channel.mention} has been removed from the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description=f"{channel.mention} is not in the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def unignore_role(self, ctx, role: discord.Role):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="<:3365:1373021326691930202> Success", description=f"{role.mention} has been removed from the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description=f"{role.mention} is not in the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def disable(self, ctx):
        ...
        if view.value is None:
            ...
            embed.title = "<:3365:1373021326691930202> Automod Disabled"
    ...
    async def logging(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            await db.commit()
            embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"**<:3365:1373021326691930202> | Automoderation Logging channel set to {channel.mention}.**\n\n➜ Use `{ctx.prefix}automod config` to view current Automod settings.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
