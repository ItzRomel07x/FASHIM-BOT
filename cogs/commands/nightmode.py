...
class Nightmode(commands.Cog):
    ...
    async def enable_nightmode(self, ctx):
        if ctx.guild.member_count < 50:  
            return await ctx.send(embed=discord.Embed(title="<:xo_cross:1373021326691930202> Access Denied",
                color=self.color,
        ...
        if not manageable_roles:
            return await ctx.send(embed=discord.Embed(title="<:xo_cross:1373021326691930202>  Error",
                color=self.color,
