...
tick = "<:3365:1373021326691930202>"
cross = "<:xo_cross:1373021326691930202>"
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"Romel Uptime", color=self.color)
      embed.add_field(name="__UTC__", value=f"<:Romel_info:1227625594511163537> {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"<:Romel_duration:1251052632949395538> {uptime_duration_string}", inline=False)
  ...
  async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <:33379crownred:1379470104672211024> :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        if ctx.guild.features:
            features = "\n".join([f"<:9387verifiedred:1379486425711640657> {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in ctx.guild.features])
            embed.add_field(
                name="**__Features__**",
        ...
        embed.add_field(
            ...
            value=f"Level: {ctx.guild.premium_tier} [<:7773shinyredpartner:1373016139017224292> {ctx.guild.premium_subscription_count} boosts]",
            inline=False
        )
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:3365:1373021326691930202> Yes' if member.bot else '<:xo_cross:1373021326691930202> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts(self, ctx):
    await ctx.send(
      embed=discord.Embed(title=f"<:7773shinyredpartner:1373016139017224292> Boosts Count Of {ctx.guild.name}",
