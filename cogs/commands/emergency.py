...
""")
    ...
    async def enable(self, ctx):
        Romel = ['213347081799073793', '677952614390038559']
        if ctx.author.id != ctx.guild.owner_id and str(ctx.author.id) not in Romel:
            embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="Only the server owner can enable emergency mode.", color=0x000000)
        ...
        if roles_added:
            ...
            embed = discord.Embed(title="<:3365:1373021326691930202> Success", description=f"The following roles with dangerous permissions have been added to the **emergency list**:\n{description}", color=0x000000)
            ...
            embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="No new roles with dangerous permissions were found.", color=0x000000)
    ...
    async def disable(self, ctx):
        Romel = ['213347081799073793', '677952614390038559']
        if ctx.author.id != ctx.guild.owner_id and str(ctx.author.id) not in Romel:
            embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="Only the server owner can disable emergency mode.", color=0x000000)
            return await ctx.reply(embed=embed)
        ...
        embed = discord.Embed(title="<:3365:1373021326691930202> Success", description="Emergency mode has been disabled, and all emergency roles have been cleared.", color=0x000000)
        await ctx.reply(embed=embed)
    ...
    async def authorise_add(self, ctx, member: discord.Member):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="Only the server owner can add authorised users for executing emergency situation.", color=0x000000)
            return await ctx.reply(embed=embed)
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if count >= 5:
                embed = discord.Embed(title="<:7942redmodshield:1373016146218848427> Access Denied", description="Only up to 5 authorised users can be added.", color=0x000000)
                return await ctx.reply(embed=embed)

            async with db.execute("SELECT 1 FROM authorised_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, member.id)) as cursor:
                if await cursor.fetchone():
                    embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="This user is already authorised.", color=0x000000)
                    return await ctx.reply(embed=embed)

            await db.execute("INSERT INTO authorised_users (guild_id, user_id) VALUES (?, ?)", (ctx.guild.id, member.id))
            await db.commit()

        embed = discord.Embed(title="<:3365:1373021326691930202> Success", description=f"**{member.display_name}** has been authorised to use `emergency-situation` command.", color=0x000000)
        await ctx.reply(embed=embed)
    ...
    async def authorise_remove(self, ctx, member: discord.Member):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="<:7942redmodshield:1373016146218848427> Access Denied", description="Only the server owner can remove authorised users for emergency situation.", color=0x000000)
            return await ctx.reply(embed=embed)

        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT 1 FROM authorised_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, member.id)) as cursor:
                if not await cursor.fetchone():
                    embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="This user is not authorised.", color=0x000000)
                    return await ctx.reply(embed=embed)

            await db.execute("DELETE FROM authorised_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, member.id))
            await db.commit()

        embed = discord.Embed(title="<:3365:1373021326691930202> Success", description=f"**{member.display_name}** has been removed from the authorised list and can no more use `emergency-situation` command.", color=0x000000)
        await ctx.reply(embed=embed)
    ...
    async def list_authorized(self, ctx):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="<:7942redmodshield:1373016146218848427> Access Denied", description="Only the server owner can view the list of authorised users for emergency situation.", color=0x000000)
            return await ctx.reply(embed=embed)
    ...
    async def role_add(self, ctx, role: discord.Role):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT COUNT(*) FROM emergency_roles WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                count = (await cursor.fetchone())[0]
            if count >= 25:
                embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="Only up to 25 roles can be added.", color=0x000000)
                return await ctx.reply(embed=embed)

            async with db.execute("SELECT 1 FROM emergency_roles WHERE guild_id = ? AND role_id = ?", (ctx.guild.id, role.id)) as cursor:
                if await cursor.fetchone():
                    embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="This role is already in the emergency list.", color=0x000000)
                    return await ctx.reply(embed=embed)

            await db.execute("INSERT INTO emergency_roles (guild_id, role_id) VALUES (?, ?)", (ctx.guild.id, role.id))
            await db.commit()

        embed = discord.Embed(title="<:3365:1373021326691930202> Success", description=f"**{role.name}** has been **added** to the emergency list.", color=0x000000)
        await ctx.reply(embed=embed)
    ...
    async def role_remove(self, ctx, role: discord.Role):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="<:7942redmodshield:1373016146218848427> Access Denied", description="Only the server owner can remove roles from emergency list.", color=0x000000)
            return await ctx.reply(embed=embed)

        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT 1 FROM emergency_roles WHERE guild_id = ? AND role_id = ?", (ctx.guild.id, role.id)) as cursor:
                if not await cursor.fetchone():
                    embed = discord.Embed(title="<:xo_cross:1373021326691930202> Error", description="This role is not in the emergency list.", color=0x000000)
                    return await ctx.reply(embed=embed)

            await db.execute("DELETE FROM emergency_roles WHERE guild_id = ? AND role_id = ?", (ctx.guild.id, role.id))
            await db.commit()

        embed = discord.Embed(title="<:3365:1373021326691930202> Success", description=f"**{role.name}** has been removed from the emergency list.", color=0x000000)
        await ctx.reply(embed=embed)
    ...
    async def list_roles(self, ctx):
        if not await self.is_guild_owner_or_authorised(ctx):
            embed = discord.Embed(title="<:7942redmodshield:1373016146218848427> Access Denied", description="You are not authorised to view list of roles for emergency situation.", color=0x000000)
            return await ctx.reply(embed=embed)
    ...
    async def emergencysituation(self, ctx):
        ...
        if not await self.is_guild_owner_or_authorised(ctx) and str(ctx.author.id) not in Romel:
        ...
        if not emergency_roles:
            ...
            return await ctx.reply(embed=discord.Embed(
                title="<:xo_cross:1373021326691930202> Error",
    ...
    async def emergencyrestore(self, ctx):
        ...
        await ctx.reply(embed=discord.Embed(
            ...
            description=f"**<:Ztick:1222750301233090600> Permissions Restored**:\n{success_message}\n\n**<:ml_cross:1204106928675102770> Errors**:\n{error_message}\n\n<:RomelArrow:1297341001341599797> Database of previously disabled permissions has been cleared.",
            color=0x000000))
