    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if log_channel:
            embed = discord.Embed(
                ...
                description=f"**<:RomelUser:1294654665895579721> User**: [{self.user}](https://discord.com/users/{self.user.id})\n**<:RomelMention:1294654604998475856> User Mention**: {self.user.mention}\n**<:RomelID:1294654633821863967> ID**: {self.user.id}\n\n**<:RomelMod:1295601558985379852> Added By**: [{self.author.display_name}](https://discord.com/users/{self.author.id})\n<:RomelTime:1294654567539277824> **Expiry Time**: {expiry_text}\n<:RomelArrow:1297341001341599797> **Timestamp**: {expiry_timestamp}\n\n<a:premium:1204110058124873889> **Tier**: **{self.values[0].upper()}**",
                color=0x000000
            )
        ...
        embed = discord.Embed(description=f"**Added Global No Prefix**:\n<:RomelUser:1294654665895579721> **User**: **[{self.user}](https://discord.com/users/{self.user.id})**\n<:RomelMention:1294654604998475856> **User Mention**: {self.user.mention}\n<:RomelID:1294654633821863967> **User ID**: {self.user.id}\n\n__**Additional Info**__:\n<:RomelMod:1295601558985379852> **Added By**: **[{self.author.display_name}](https://discord.com/users/{self.author.id})**\n<:RomelTime:1294654567539277824> **Expiry Time:** {expiry_text}\n<:RomelArrow:1297341001341599797> **Timestamp:** {expiry_timestamp}", color=0x000000)
...
class NoPrefix(commands.Cog):
                    ...
                    if user:
                        ...
                        if log_channel:
                            embed_log = discord.Embed(
                                ...
                                description=(
                                    f"**<:RomelUser:1294654665895579721> User**: [{user}](https://discord.com/users/{user.id})\n"
                                    f"**<:RomelMention:1294654604998475856> User Mention**: {user.mention}\n"
                                    f"**<:RomelID:1294654633821863967> ID**: {user.id}\n\n"
                                    f"**<:RomelMod:1295601558985379852> Removed By**: **[Romel#9545](https://discord.com/users/1144179659735572640)**\n"
                                ),
                                color=0x000000
                            )
                        ...
                        embed.set_footer(text="Romel - No Prefix, Join support to regain access.")
    ...
    async def np_add(self, ctx, user: discord.User):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if result:
                embed = discord.Embed(description=f"**{user}** is Already in No prefix list\n\n<:RomelMod:1295601558985379852> **Requested By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n", color=0x000000)
    ...
    async def np_remove(self, ctx, user: discord.User):
        ...
        embed = discord.Embed(
                description=(
                    f"**<:RomelUser:1294654665895579721> User**: [{user}](https://discord.com/users/{user.id})\n"
                    f"**<:RomelMention:1294654604998475856> User Mention**: {user.mention}\n"
                    f"**<:RomelID:1294654633821863967> User ID**: {user.id}\n\n"
                    f"**<:RomelMod:1295601558985379852> Removed By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n"
                ),
            color=0x000000
        )
        ...
        async with aiosqlite.connect('db/np.db') as db:
            ...
            if not result:
                embed = discord.Embed(
                    ...
                    description=f"**{user}** is Not in the No Prefix list\n\n"
                                f"<:RomelMod:1295601558985379852> **Requested By**: "
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"**<:RomelUser:1294654665895579721> User**: [{user}](https://discord.com/users/{user.id})\n"
                    f"**<:RomelID:1294654633821863967> User ID**: {user.id}\n\n"
                    f"**<:RomelTime:1294654567539277824> Expiry**: {expire_time} ({expire_timestamp})"
                ),
                color=0x000000
            )
    ...
    async def add_np(self, user, duration):
        ...
        embed = discord.Embed(
                            title="<:Romel_giveaway:1243956246961459220> Congratulations you got 2 months No Prefix!",
