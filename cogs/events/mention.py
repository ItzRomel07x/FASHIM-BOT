    ...
    async def on_message(self, message):
        ...
        if self.bot.user in message.mentions:
            if len(message.content.strip().split()) == 1:
                ...
                embed.set_footer(text="Powered by Romel Development™", icon_url=self.bot.user.avatar.url)

                buttons = [
                    discord.ui.Button(label="Invite", style=discord.ButtonStyle.link, url="<https://discord.com/oauth2/authorize?client_id=1368824214274048122&permissions=8&integration_type=0&scope=bot"),
                    discord.ui.Button(label="Web", style=discord.ButtonStyle.link, url="https://olyumpus.vercel.app/"),
                    discord.ui.Button(label="Support", style=discord.ButtonStyle.link, url="https://discord.gg/9jmCp9cnRc"),
                ]
