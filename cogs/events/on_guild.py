...
class Guild(Cog):
    ...
    async def on_guild_add(self, guild):
        try:
            ...
            embed.add_field(
                name="**__Members__**",
                value=f"""<:romel_users:1226064812157571163> Members : {len(guild.members)}\n<:romel_friend:1228229690376982549> Humans : {len(list(filter(lambda m: not m.bot, guild.members)))}\n<:romel_bot:1227873668466741320> Bots : {len(list(filter(lambda m: m.bot, guild.members)))}
                """,
                inline=False
            )
            embed.add_field(
...
Threads : {len(guild.threads)}
    ...
    color=0xff0000)
            ...
            embed.set_footer(text="Powered by FAHIM CHEATS™", icon_url="https://cdn.discordapp.com/icons/699587669059174461/f689b4366447d5a23eda8d0ec749c1ba.png")
            if guild.icon:
                embed.set_thumbnail(url=guild.icon.url)

            support = Button(label='Support',
                             style=discord.ButtonStyle.link,
                    url=f'https://discord.gg/xrzyjeKha2')
            web = Button(label='Website',
                             style=discord.ButtonStyle.link,
                    url=f'https://my-web-dw7c.onrender.com')
            view = View()
    ...
    async def on_guild_remove(self, guild):
        try:
            ...
            embed.add_field(
                name="**__About__**",
                value=f"**Name : ** {guild.name}\n**ID :** {guild.id}\n**Owner <:FAHIM CHEATS_owner:1243891618897989675> :** {guild.owner} (<@{guild.owner_id}>)\n**Created At : **{guild.created_at.month}/{guild.created_at.day}/{guild.created_at.year}\n**Members :** {len(guild.members)}",
                inline=False
            )
            embed.add_field(
                name="**__Description__**",
